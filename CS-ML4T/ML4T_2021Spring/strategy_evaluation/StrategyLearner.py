...
class StrategyLearner(object):
    ...
    def __init__(self, verbose=False, commission=9.95, impact=0.005):
        ...
        if self.verbose:
            ...
            print('Bag Learner with 25 Random Tree Learners and 5 leaf_size')
    ...
    def testPolicy(
        ...
        if self.verbose:
            print('trades type ', type(trades))  # it better be a DataFrame!
        if self.verbose:
            print('orders - \n', trades)
    ...
    def testStrategy(self, symbol="JPM", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 12, 31), sv=100000):
        orders = self.testPolicy(symbol, sd, ed, sv)
        benchmark = get_benchmark(symbol, sd, ed, sv)
        portvals = compute_portvals(symbol, orders, sv, commission=9.95, impact=0.005)

...
class RTLearner(object):
    ...
    def __build_tree(self, data):
        if data.shape[0] <= self.leaf_size:
            m = stats.mode(data[:, -1])[0][0]
            return np.array([[-1, m, -1, -1]])
        unique_y = np.unique(data[:, -1])
        if unique_y.shape[0] == 1:
            return np.array([[-1, unique_y[0], -1, -1]])

        num_features = list(range(data.shape[1] - 1))
        attempt = 0
        while len(num_features) > 0:
            best_feature = random.choice(num_features)
            i, j = random.sample(range(0, data.shape[0]), 2)
            SplitVal = np.mean([data[i, best_feature], data[j, best_feature]])
            ...
            num_features.remove(best_feature)
            attempt += 1
